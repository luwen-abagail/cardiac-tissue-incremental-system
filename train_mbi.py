    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in  dataloader:
            ...
            if teachermodel == None:
                ...
                # 有teacher模型时的蒸馏损失
                with torch.no_grad():
                    _, teacher_out = teachermodel(images)
                    
                # 计算学生模型的输出
                _, student_out = studentmodel(images)
                dice=diceloss(student_out, labels)
                student_out=student_out[:, :num_classes-1, :, :]
                # 知识蒸馏损失
                kd_loss = F.kl_div(
                    F.log_softmax(student_out/3.0, dim=1),
                    F.softmax(teacher_out/3.0, dim=1),
                    reduction='batchmean'
                ) 
                    
                
                
                loss = (1-distillation_weight) * dice + distillation_weight * kd_loss
