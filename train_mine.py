    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in  dataloader:
            ...
            if teachermodel == None:
                ...
                if epoch == 0:
                    patch_list = _get_patches(images, labels, num_classes, 256, patch_size, patchnum)
                    patch.append(patch_list)
                else:
                    patch_list=patch[progress-1]
                student_emb_list = get_embeddings(fea, patch_list, patch_size,patchnum) 
                teacher_emb_list=[]
                # 计算损失
                #enqueue these

	            #calculate PCGJCL loss,patch，对比损失
                PCGJCL_loss =PCGJCL(student_emb_list,teacher_emb_list,0.2 ,patchnum)
                loss =0.99*supervised_loss+0.1*PCGJCL_loss 
            else:
                ...
                if epoch == 0:
                    ...
                    patch_list=patch[progress-1]
                student_emb_list = get_embeddings(fea, patch_list, patch_size,patchnum) 
                teacher_emb_list = get_embeddings(feaold, patch_list, patch_size,patchnum) 
                # 计算损失
                #enqueue these
                ...
                loss =0.9*supervised_loss+0.1*PCGJCL_loss 
                #print(PCGJCL_loss)
                #print(loss)
            ...
            with torch.no_grad():  # 禁用梯度计算
                ...
                for images, labels in dataloader1:
                    ...
                    '''if  epoch%30==0:
                        if progress==1:
                            fea_t.append(fea[0])
                            fea_t.append(F.interpolate(fea[1], scale_factor=2, mode='nearest'))
                            fea_t.append(F.interpolate(fea[2], scale_factor=4, mode='nearest'))
                            fea_t.append(F.interpolate(fea[3], scale_factor=8, mode='nearest'))
                            feat_2t.append(torch.cat(fea_t, dim=0))
                            label_t.append(labels)
                        fea_2t= torch.stack(feat_2t, dim=0).permute(1,0,2,3,4)
                        label_2t=torch.cat(label_t, dim=0)
                        label_2t=label_2t.view(-1).cpu().numpy()
                        fea_2t=fea_2t.reshape(fea_2t.shape[0],-1).permute(1,0)
                        tsne = TSNE(n_components=2, random_state=42)
                        features_low_dim = tsne.fit_transform(fea_2t.cpu().numpy())
                        max_samples_per_class = 200
                        unique_labels = np.unique(label_2t)
                        sampled_indices = []

                        for label in unique_labels:
                            class_indices = np.where(label_2t == label)[0]
                            if len(class_indices) > max_samples_per_class:
                                sampled_indices.extend(
                                    np.random.choice(class_indices, size=max_samples_per_class, replace=False)
                                )
                            else:
                                sampled_indices.extend(class_indices)

                        # 使用欠采样后的数据
                        features_sampled = features_low_dim[sampled_indices]
                        labels_sampled = label_2t[sampled_indices]
                        # 可视化降维后的特征
                        plt.figure(figsize=(10, 8))
                        scatter = plt.scatter(features_sampled[:, 0], features_sampled[:, 1], c=labels_sampled, cmap='viridis', alpha=0.7,s=2)
